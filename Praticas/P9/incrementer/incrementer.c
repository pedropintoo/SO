...
int main (int argc, char *argv[])
{
    ...
    for (i = 0; i < nthr; i++) {
        if (pthread_create (thr+i, NULL, incrementer, &niter) != 0) {
            // Thread with id = *(thr+1), default atrivutes, will run function 'increment' with arg niter 
    ...
    for (i = 0; i < nthr; i++) { 
        // "Recolher" as threads, "volta a casa" - by guilherme
